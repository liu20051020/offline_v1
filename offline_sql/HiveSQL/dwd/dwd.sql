...
insert overwrite table dwd_trade_cart_add partition (ds = '20250918')
select
    id,
    user_id,
    sku_id,
...
insert overwrite table dwd_trade_cart partition(ds='20250918')
select
    id,
    user_id,
    sku_id,
...
(
    ...
    PARTITIONED BY (`ds` STRING)
    STORED AS ORC
    LOCATION '/warehouse/gmall/dwd/dwd_traffic_page_view'
    TBLPROPERTIES ('orc.compress' = 'snappy');

insert overwrite table dwd_traffic_page_view partition (ds='20250918')
select
...
DROP TABLE IF EXISTS dwd_user_register;
...
(
    ...
    PARTITIONED BY (`ds` STRING)
    STORED AS ORC
    LOCATION '/warehouse/gmall/dwd/dwd_user_register/'
    TBLPROPERTIES ("orc.compress" = "snappy");

set hive.exec.dynamic.partition.mode=nonstrict;
insert overwrite table dwd_user_register partition(ds)
select
    ui.user_id,
...
(
    ...
    PARTITIONED BY (`ds` STRING)
    STORED AS ORC
    LOCATION '/warehouse/gmall/dwd/dwd_user_login/'
    TBLPROPERTIES ("orc.compress" = "snappy");
